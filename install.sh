...
if test "$NETWORK_RC_VERSION" = ""; then
  echo '安装最新版本'
   DOWNLOAD_LINK="git clone https://github.com/qyr001/network-rc.git"
else
...
if [ "$ok" = "ok" ]; then
  ...
  echo ""
  echo ""
  sudo rm -rf /home/pi/network-rc
  if test "$NETWORK_RC_BETA" = "1"; then
    echo "下载 Network RC beta 版本"
    if [ ! -d "/home/pi/network-rc/" ]; then
      git clone https://github.com/qyr001/network-rc.git
      echo "下载成功"
    else
      echo "下载失败"
      exit 1
